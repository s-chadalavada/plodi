...
* Case Data - We reviewed and labelled 108 adjudicated DOJ cases. 108 cases. 614 individual and company names yielding 752 unique loans. Assume that a loan is “suspect” if it’s associated with one of the cases (data labelling) (Link here)[http://www.justice.gov/criminal/criminal-fraud/cares-act-fraud]^2^

## Modeling & Methodology {#modeling}
We assume a true fraud rate of 8% as estimates range from $70B-$200B of the $1.2T disbursed. All models are trained/tested on ~9.4k loans, via downsampling of the non-case related loans, and assumed to be non-suspect with a train and , wTest split of 80% and 20% respectively. Given prosecuted cases are positive loan labels but remaining loans are unknown statuse weigh Recall (Sensitivity) and Negative Predictive Value as primary measures for MVP model selection. Based on the results from our test data, we selected XGBoost as our champion model as it outperformed relative to the other supervised and semi-supervised models we tested, which is consistent from prior published outlier detection benchmarks^3^. For more information on our model’s test results, please see the [presentation linked](https://www.ischool.berkeley.edu/sites/default/files/sproject_attachments/plodi_-_mids_w210_fall_2023_sec_9.pdf).
